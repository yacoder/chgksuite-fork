...
По умолчанию экспорт уменьшает размер презентации: растровые картинки пережимаются в JPEG с качеством 80, если новый файл получается меньше. Можно отключить это через `--optimize_size off`.
...
Если указать `disable_shrink_fit = true`, экспорт не включает автоматическое уменьшение текста в PowerPoint, но продолжает использовать размеры шрифта из конфигурации. В `[font]` можно отдельно задать `default_size`, `question_size`, `answer_size`, `number_size` и `title_size`. Размер из `[number_textbox].font_size`, если он указан, важнее `font.number_size`; старые поля `force_text_size_question` и `force_text_size_answer` продолжают работать как `font.question_size` и `font.answer_size`. Для особого управления межстрочным интервалом можно указать `fixed_line_spacing_question = 24` и `fixed_line_spacing_answer = 20`; тогда при включённом shrink-fit межстрочный интервал вопроса и ответа фиксируется в этих абсолютных pt. Если абсолютный интервал не задан, можно указать `line_spacing_multiplier = 1.2`, и PowerPoint получит фиксированный множитель 120%. По умолчанию эти параметры отсутствуют, и PowerPoint shrink-fit работает штатно, в том числе может менять межстрочный интервал. Если указать `overlay_image_and_text = true`, картинка и текст вопроса помещаются в одну и ту же область слайда, так что текстовый блок может накладываться на картинку.
