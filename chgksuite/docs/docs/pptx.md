...
В нём вас главным образом интересует поле `template_path` — это путь к презентации, которая используется в качестве шаблона. Если вы хотите использовать, например, свой фон — установите его в презентации и укажите путь к ней в `template_path`, затем сохраните файл конфигурации с расширением `.toml` и попробуйте запустить экспорт с ним. Возможно, вам захочется подвигать поле с текстом или номером вопроса — это можно делать с помощью полей `left` и `top` в `[textbox]` и `[number_textbox]`, соответственно. Шрифт можно задать в конфигурации или переопределить при запуске с помощью `--font`; при `--embed_fonts on` выбранный шрифт встраивается в PPTX.

Если указать `disable_shrink_fit = true`, экспорт не включает автоматическое уменьшение текста в PowerPoint, но продолжает использовать размеры шрифта из конфигурации. В `[font]` можно отдельно задать `default_size`, `question_size`, `answer_size`, `number_size`, `title_size` и `tour_size`. Размер из `[number_textbox].font_size`, если он указан, важнее `font.number_size`; старые поля `force_text_size_question` и `force_text_size_answer` продолжают работать как `font.question_size` и `font.answer_size`. Для особого управления межстрочным интервалом можно указать `fixed_line_spacing_question = 24` и `fixed_line_spacing_answer = 20`; тогда при включённом shrink-fit межстрочный интервал вопроса и ответа фиксируется в этих абсолютных pt. Если абсолютный интервал не задан, можно указать `line_spacing_multiplier = 1.2`, и PowerPoint получит фиксированный множитель 120%. По умолчанию эти параметры отсутствуют, и PowerPoint shrink-fit работает штатно, в том числе может менять межстрочный интервал. Если указать `overlay_image_and_text = true`, картинка и текст вопроса помещаются в одну и ту же область слайда, так что текстовый блок может накладываться на картинку.

Если нужно отключить PowerPoint shrink-fit и подбирать размер шрифта заранее, можно задать сетку размеров:
